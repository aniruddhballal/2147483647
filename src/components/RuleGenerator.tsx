...
const ruleToOutputs = (ruleNum: number): number[] =>
  PATTERNS.map((_, i) => (ruleNum >> (7 - i)) & 1);

const outputsToRule = (outputs: number[]): number =>
  outputs.reduce((acc, bit, i) => acc + (bit << (7 - i)), 0);

const CELL_SIZE = 6;
const COLS = 120;

const pillBtn = (bg: string): React.CSSProperties => ({
  background: bg.startsWith("#") ? bg + "22" : bg,
  border: `1px solid ${bg.startsWith("#") ? bg + "66" : "rgba(255,255,255,0.15)"}`,
  color: bg.startsWith("#") ? bg : "#fff",
...
export default function RuleGenerator() {
  const [outputs, setOutputs] = useState<number[]>(() => ruleToOutputs(30));
  ...
  const rowsRef = useRef<Uint8Array[]>([]);
  ...
  useEffect(() => {
    ...
    canvas.height = Math.max(r.length * CELL_SIZE, 20);
  ...
  const stepOnce = useCallback(() => {
    ...
    for (let c = 0; c < COLS; c++) {
      ...
      next[c] = outputs[patternIndex(a, b, cv)];
    }
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center" }}>
        <h1 style={{ margin: 0, fontSize: "22px", fontWeight: 700, letterSpacing: "0.08em" }}>
          Rule Generator
        </h1>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "center" }}>
        {PATTERNS.map(([a, b, c], i) => (
          <div key={i} style={{
            ...
            borderRadius: "10px", padding: "10px 10px 8px", minWidth: "52px",
          }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{
          ...
          {(["manual", "auto"] as const).map((m) => (
            <button key={m} onClick={() => { setMode(m); setIsRunning(false); }} style={{
        ...
        {mode === "manual" && (
          <button onClick={stepOnce} style={pillBtn("#f0a050")}>Next →</button>
        )}

        {mode === "auto" && (
          <>
            <button onClick={() => setIsRunning((r) => !r)}
              style={pillBtn(isRunning ? "rgba(255,80,80,0.8)" : "#f0a050")}>
              {isRunning ? "⏸ Pause" : "▶ Run"}
            </button>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", letterSpacing: "0.08em" }}>SPEED</span>
              <input type="range" min={20} max={500} step={10} value={speed}
                onChange={(e) => setSpeed(Number(e.target.value))}
                style={{ accentColor: "#f0a050", width: "90px" }} />
              <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", minWidth: "40px" }}>{speed}ms</span>
            </div>
          </>
        )}
      </div>
      ...
      <div ref={scrollRef} style={{
        ...
        overflow: "auto", maxHeight: "380px", maxWidth: "100%",
        background: "#0b0b0f", scrollbarWidth: "thin",
